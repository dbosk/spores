...
\begin{figure}
  ...
  \begin{algorithmic}[1]
    ...
    \Function{\CreateHeader}{$I, L$}
      ...
      \Comment{Maximum width of a layer, \ie alternative nodes.}
      ...
      \State $\phi_1, \dotsc, \phi_\nu\gets \ComputePadding[\{s_1, \dotsc, 
      s_\nu\}, w_H]$
    ...
    \Function{\WrapHeader}{$i, (\alpha_{i+1}, \beta_{i+1}, \gamma_{i+1}), 
      ...
      \Return $i-1, (\alpha_i, C_i, \beta_i, \gamma_i)$
    \EndFunction
  \end{algorithmic}
  \end{minipage}}
...
\begin{figure}
  \framebox{\begin{minipage}{0.96\linewidth}
  \begin{algorithmic}[1]
    \Function{\ComputePadding}{$\{s_1, \dotsc, s_\nu\}, w_H$}
      \State $\phi_1\gets \epsilon$
      \Comment{$\epsilon$ is the empty string.}
      \For{$2\leq i\leq \nu$}
        \State $\phi_i = \{ \phi_{i-1}\concat 0_{w_H} \} \oplus
          \{ \prg(\hash_\prg(s_{i-1}))_{[(w_H(r-i+1)+3)\kappa..(w_H 
            r+3)\kappa-1]} \}$
      \EndFor
      \State \Return $\phi_1, \dotsc, \phi_\nu$
    \EndFunction
  \end{algorithmic}
  \end{minipage}}
