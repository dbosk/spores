...
\sonja{Daniel is working on sketching a proof/rephrasing the following.}
...
\paragraph*{Security of \ac{SPORES}}

\Ac{SPORES} inherits the security properties from Sphinxes and \ac{SPOR}.
...
\Ac{SPORES} also inherits a vulnerability from random-walk protocols and the 
fact that a large message is split into chunks.
Since the chunks can take several paths from source to destination, the source 
will send and the destination will receive more traffic than other nodes in the 
network~\cite{RoutingSurveyAnonymousProtocols} \commentDaniel{we should 
  probably look for the original papers}.
However, this is slightly alleviated by the fact that the source and 
destination can be several different devices --- \ie from the \squad{}s.
...
with whom~\cite{StatisticalDisclosureAttacks}. \sonja{Daniel: the
  reference seems to be missing}  There are also denial
...
adversary has introduced an error.  \sonja{``But these advantages will
  ...
  this denial of security.} We would also like to point out that
...
would render traffic analysis pointless.
