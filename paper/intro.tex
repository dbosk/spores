...
Second, we provide a protocol, \ac{SPOR}, which optimizes the layers for 
availability.
See \cref{SPOR} for details.
We use a wide definition of churn, a node that is available is online and not 
congested.
This allows systems to relay messages in a \ac{P2P} network with high churn.
For instance, one could convert Signal to use \ac{SPOR} for message delivery.
We use this as the scenario for our performance evaluation.
