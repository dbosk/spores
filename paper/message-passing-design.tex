\subsection{Message Passing through \acl*{SPOR}}%
\label{sec:message_passing}

We have \iac{SPOR} scheme, which provides probabilistically onion-routed message 
passing.
Instead of providing only one address per hop in the route (as in \eg 
Tor~\cite{Tor} or remailers), \ac{SPOR} provides several alternatives for the 
next hop depending on their probability of being online.
To do this, the \ac{SPOR} scheme provides stateless algorithms for the 
onion-routing.
...
The \(\SPOR\) scheme provides three algorithms: \(\SPOR[\Send]\) 
(\cref{SPORSend}), \(\SPOR[\Fwd]\) (\cref{SPORFwd}) and \(\SPOR[\Recv]\) 
(\cref{SPORRecv}).
