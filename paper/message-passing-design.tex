...
\commentAL{Is it good?}
In \name, whole messages, headers and file chunks are considered having a fixed size.
In reality, the message size \messagesize is indeed fixed to prevent \name from message size analysis attacks,
but the header size \headersize is variable, depending on the number of layers and the number of devices per layer.
File chunks would then have a size  inversely proportional to \headersize : $\filesize = \messagesize - \headersize$. 
