...
\NewVariable{\Hforward}{\mathcal{H}_{\text{forward}}\xspace}
\NewVariable{\Hbackward}{\mathcal{H}_{\text{backward}}\xspace}
\NewVariable{\Hsendforward}{\mathcal{H}_{\text{send forward}}\xspace}
\NewVariable{\Hsendbackward}{\mathcal{H}_{\text{send backward}}\xspace}
\NewVariable{\Hrecforward}{\mathcal{H}_{\text{rec forward}}\xspace}
\NewVariable{\Hrecbackward}{\mathcal{H}_{\text{rec backward}}\xspace}
...
She also build two headers: \Hsendforward contains the route from her \squad (excluded) to \LRV (included), while \Hsendbackward contains the route from \LRV (excluded) to her \squad (included).
Alice finally sends \Hsendbackward back to Bob: he is going to need it to sends acknowledgments back to Alice.
...
\begin{figure}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{\Recv}{sender}
      ...
      % \State \Hsendforward \gets sender.\Send(\Hrecforward, \LRV)
      % \State \Hbackward \gets \Hrecbackward || \Hsendbackward
