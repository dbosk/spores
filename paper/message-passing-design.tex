...
\begin{figure}[tbh]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{\Recv}{$D$}
      \State $H_\rdv\gets \ExtendRoute[D\concat \changeBisFT{\DeBEenc[\mpk, D, \top]}, L, \theta]$
      \State \Return $H_\rdv$
    ...
    \Require{%
      $m$, the message, and
      $H_\rdv$, the onion route to the recipient
    }
    \Function{\Send}{$H_\rdv,m$}
      \State $H\gets \ExtendRoute[H_\rdv, L, \theta]$
      \State $D\concat C_H\gets H$  \changeBisFT{\Comment{Grabbing first hop}}
      \For{$d\in D$}
        \Comment{Uniformly randomly chosen}
        % FT10Jul18: \Fwd is now an asynchronous RPS, so no return value
        \State \changeBisFT{$d\method \Fwd[C_H, m]$ \Comment{Asynchronous RPC}}
    ...
    \Function{\Fwd}{$C_H, m$}
      ...
      % FT10Jul18: the \bot and \top return values do not seem to be
      % used, so dropping them
      \If{\changeBisFT{$H = \top$}}
        \changeBisFT{\Comment{End of the route, the current node $\in D$}}
        \State \changeBisFT{$\Store[m]$}
      \Else
        \State $\{d_i\}\concat C_H'\gets H$ \changeBisFT{\Comment{Grabbing next hop}}
        %\If{$C_H' = \top$}
...
%      \State \Return $\bot$
    \EndFunction
    \Function{\Store}{$m$}
      \State $\changeBisFT{m_{\mathrm{clear}}}\gets \DeBEdec[\mpk, \sk, \changeBisFT{m}]$
      %% \If{$m = \bot$}
      %%   \State \Return $\bot$
      %% \EndIf
      \State Store $\changeBisFT{m_{\mathrm{clear}}}$ to disk.
%      \State \Return %$\top$
    \EndFunction
  \end{algorithmic}
  \end{minipage}}
