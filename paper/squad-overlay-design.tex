\subsection{The E-Squad Overlay} 
...
We show in section~\label{sub:predictive_onion_routes_creation} that the prediction of their availability at $t+1$ is a good enough estimator of their connection state at $t+i$.
For that reason, devices only share $P\left[ d \in O_{t+1} \mid S_{t} = s \right]$ with other devices for PORs creation.
