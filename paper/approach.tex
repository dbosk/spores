...
categories: (i) \textit{network structure}, (ii) \textit{routing information},
(iii) \textit{communication model} and (iv) \textit{performance} (see~\cite[Table 1]{RoutingSurveyAnonymousProtocols}).
...
Finally, we introduce the complete \ac{SPORES} protocol, which 
allows the device \squad to act as an autonomous system (Figure~\ref{fig:outline} \ding{204}).
