...
inferences from metadata even when the data itself is encrypted, and
thus poses a threat to the users' privacy.
...
provider, as we propose to do, removes the privacy risks associated
with a central brokering entity, but also removes the gatekeeping a
cloud provider provides, and exposes users to friendly
surveillance~\cite{FriendlySurveillance}, in an honest-but-curious
setting, by other peers (other \name users including Alice and
Bob). In particular other users might collude to observe some of the
traffic, and gain knowledge regarding Alice's and Bob's interactions,
the devices they own, and how they use them.
...
typically associated with onion routing, as defined by Camenisch and Lysianskay~\cite{CLOnionRouting}: 
...
can be infered through \name.  \commentFT{Removed, as did not fit with
...
well as some of the nodes. We do not consider a passive global network
