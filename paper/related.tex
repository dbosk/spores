...
Cloud services for availability of files on multiple devices and sharing with others, such as Dropbox, Box, or iCloud, require trust in the service provider, at minimum to keep the service running without changes to the terms of service becoming unacceptable. Alternatively, users could set up their own cloud using a dedicated server or edge devices such as home gateways. This would require devices that users are less likely to already have than laptops or smartphones. P2P file sharing has been researched extensively, with respect to aspects such as architecture (starting with the semi-centralized Napster~\cite{napster}), availability and replication~\cite{gnutella, krz}, scalability~\cite{bittorrent}, anonymity~\cite{freenet, Tor}, and distributed hash tables (DHT)~\cite{opendht, can, chord, pastry}. In contrast to our work, the main goal was to enable dissemination to many users who do not necessarily know each other in person. Here, we focus on sharing files in a more deliberate yet ad-hoc manner, i.e., between users that out-of-band agree to share a file but do not keep it available for anyone else in the future. TOR~\cite{Tor}, although not designed for file sharing specifically, comes closest in that respect, but does not address the following aspect.

There is a concern shared with traditional p2p file sharing, namely churn and its effect on availability - here in the narrow context of the set of user devices based on user behavior. We thus discuss the related work on predicting user behavior next.
