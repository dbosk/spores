...
\newcommand{\localoverlay}{Squad Overlay}
\newcommand{\globaloverlay}{Global Overlay}
...
We call this part of the protocol the \localoverlay, and present it in section~\ref{local overlay}. 
On the other hand, user's collectively upload the list of their connected devices and associated future predictions to a Distributed Hash Table (DHT): it serves as a registry of online peers (including connection probabilities for the next turn), and for the routing of files.
We describe this \globaloverlay in section~\ref{global overlay}. 
File exchange between two users is enabled by the two previous protocols, and is covered in section~\ref{file exchange}
...
\begin{enumerate}
	...
	\item Share files location \commentAL{Maybe...}
\end{enumerate}
...
\commentAL{Then we \textbf{maybe} do predictions as \textbf{maybe} shown in section~\ref{sec:user_model}.}
...
\begin{enumerate}
	\item Providing routing information to nodes during file exchange. This is discussed in-depth in section~\ref{file exchange};
...
Bob fetches the addresses of Alice's IP from the DHT.
...
$$ P(pk_{r_{i,1}}, sk) || P(pk_{r_{i,2}}, sk) || P(pk_{r_{i,3}}, sk) || S(sk, R_{i+1}) $$
