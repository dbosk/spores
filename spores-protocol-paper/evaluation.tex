...
In the following, we first give precisions on the evaluation testbed and parameters in section~\ref{sub:evaluation_testbed}, before evaluating the file transfer performance in section~\ref{sub:file_transfer}. Finally, we look at the devices availability over time, and at the messages life over the network in \ref{ssub:traffic_and_availability}.
...
\subsection{Availability of nodes and messages in transit} % (fold)
...
\begin{figure}[t]
  ...
  Availability of devices (in gray) and number of messages waiting for reception (in black) over an experiment. The black dotted line represents the mean rate of online devices: 15.8\%.}
\end{figure}
...
Indeed, it is interesting to look at the transmission times of the messages: their mean time in transit is of 22s in this experiment, which is incredibly long.
...
We have seen that \name could empower users to privately exchange files in a distributed manner over a challenging network where only 15.8\% of the infrastructure is available at a time.
Our proposal allows to nevertheless complete the file exchange with an average probability of 94.4\%, against 41.7\% success rate using classical onion routes.
