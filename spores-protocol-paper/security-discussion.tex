...
\daniel{Check that we still use the \(D_i\) notation for the devices in 
  \(i\)'s squad.}
...
\daniel{Hard to say at this time of night \dots}
...
Alice is the first node and Bob is the last node. \sonja{but what can
  she learn/guess?}
\daniel{For all she knows there might be another node at either the 
...
adversary has introduced an error.  \sonja{``But these advantages will
  ...
  protocol must resend.'' why?}  \daniel{Due to statistics. We should 
  also look into
...
\daniel{we should probably look for the original papers} --- this 
vulnerability is inherited from the mixing strategy.


% \daniel{Predecessor attacks: random walk protocols has the problem that 
