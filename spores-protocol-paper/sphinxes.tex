...
Sphinx.
Since it is \acadv{UC} secure, it provably implements an ideal onion routing 
functionality.
...
\Ac{Sphinx} only provides one node per onion layer; in this section we extend \ac{Sphinx} 
...
and mixing (OR, top) and \ac{SPOR} (bottom).
    %\daniel{This figure should be fixed, issue \#42.}
  }
\end{figure}

\subsection{Overview of the Sphinx header format}

We will now give an overview of the \ac{Sphinx} header format.
...
achieve \Sphinxes.
For a more detailed understanding of \ac{Sphinx}, we refer the reader to the 
...
We will now extend \ac{Sphinx} to do \ac{SPOR}: instead of providing only \(d_{i+1}\) 
...
\subsection{\Sphinxes algorithms and \acf*{SPOR}}
